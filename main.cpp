...
void hpRepair(int &hp, int &hpmax) {
    if (hp > hpmax) {
...
void fight(int &hp, int &money, int &exp, int lvl, int &armor, int &mana, int &dmg, int &hpmax, char &mob) {
...
void endgame(int &lvl){
    if(lvl<69){
        std::cout<<"masz za niski poziom by ukończyć grę\n";
    }if(lvl>=69 && lvl<666){
        std::cout<<"udało ci sie ukończyć grę! Wszystkie podstawowe pokraki zostały zneutralizowane\n ewidentnie nie chce ci się już grać w te wybitną grę rozumiem to bardzo dobrze\n";
    }if(lvl>=666 && lvl<2137){
        std::cout<<"udało ci sie ukończyć grę! Wszystkie pokraki zostały zneutralizowane\n Belzedup mocno odczuł cios zadany przez ciebie ale czy to wystarczy by już nie powrócił?\n";
    }if(lvl>=2137){
        std::cout<<"WOOOOOOW nie spodziewałem sie, że ktoś kiedykolwiek bedzie tyle w to grać \n sam papiesz polak jest z ciebie dumny oczyściłeś całą warszawę z diabłow itp";
    }
}
...
void displayFileListAndLoadData() {
    ...
    if (chosenFileNum > 0 && chosenFileNum <= txtFiles.size()) {
        ...
        if (chosenFile.is_open()) {
            std::string line;
            std::string name;
            int hp, armor, dmg, money, hydro, level, exp, hpmax, hydromax, rasa, klasa;
            ...
            while (std::getline(chosenFile, line)) {
                 ...
                 if (line.find("HP: ") != std::string::npos) {
                    ...
                    armor = std::stoi(line.substr(6));
                } else if (line.find("Damage: ") != std::string::npos) {
                    dmg = std::stoi(line.substr(8));
...
int main() {
    ...
    char klasa, advmenu, mob, workChoice, ksChoice, msChoice, pieCHoice;
    int rasa;
    bool loopExit = false;
    bool loopGameExit = false;
    while (true) {
        ...
        std::cout << "4. zapis gry\n";
        std::cout << "99. wyjście\n";
        char menu;
        std::cin >> menu;
        if (menu == '1' && loopExit == false) {
            ...
            std::cout
                    << "Witaj w roku 2077 jesteś w Wraszawie świat został opanowany przez diabły i inne pokraki. Polska nie zmieniłą sie nic w porównaniu z rokie 2024. \n"
                       "twoim celem jest wyczyszcznie warszawy z sił nieczytych oraz pomoc obywatelom. Gra zakończy sę gdy osiagniesz poziom 69, 666 lub 2137 ty decydujesz kiedy końćzysz rozgrywkę. \n";
            loopExit = true;
        } else if (menu == '2' && loopExit == false) {
            ...
            loopExit = true;
        } else if (menu == '3' && loopExit == false) {


            // Ustawienia - W sumie chuj wie co tu dać
            break;
        } else if (menu == '4' && loopExit == false) {
            std::string saveName;
            std::cout << "Podaj nazwę save: ";
            std::cin >> saveName;
            createSaveFile(hp, armor, dmg, money, hydro, lvl, exp, hpmax, hydromax, rasa, klasa, saveName);
        } else if (menu == '5' && loopExit == false) {


        } else if (menu == '99' && loopExit == false) {
        ...
        while (loopGameExit == false) {


            checkExpLevel(exp, lvl);
            std::cout << std::endl;
            std::cout << "____________________Statystyki________________________________\n";
            std::cout << "Życie: " << hp << "/" << hpmax << " Najpojenie: " << hydro << "/" << hydromax
                      << " Pieniądze: " << money << "\n Siła ataku: " << dmg
            ...
            switch (menugame) {
                case '1':
                    std::cout
                            << "Wyruszasz w przygodę pełną niespodzianek rzuć kościa by doweidzieć sie co sie wydarzy\n";
                            ...
                            switch (choice) {
                                case '1':
                                    if (money >= 5) {
                                        ...
                                        fight(hp, money, exp, lvl, armor, hydro, dmg, hpmax, mob);
